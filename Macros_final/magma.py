...
size=100 #number of cells along one side
dim=9 #fm , the grid stretches from [-dim, +dim]
...
##################################################
############### graphic check ####################
######## check that source coords are OK #########
##################################################
...
###############################################################################
    ...
    obs[ev,6]=np.abs(E3)
    #evaluate rms radius
    obs[ev,3]=np.sqrt(np.sum(e2_den)/e_tot)

print('it took', (time.clock() - time_g), 's for ', nev, '  Pb-Pb events')
